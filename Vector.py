...
class Line:
    ...
    def get_point(self,points,alpha,deg=None):
        ...
        vector2=Vector(self.get_point(points,alpha,deg - 1).get_x(),self.get_point(points,alpha,deg - 1).get_y())
    ...
    def set_points(self,points,speeds):
        for point in range(len(points)):
            vec1=Vector(points[point].get_x(),points[point].get_y())
            vec2=Vector(speeds[point].get_x(),speeds[point].get_y())
    ...
    def draw_points(self,points,style="points",width=4,color=(255,255,255)):
        if style=="line":
            for point_number in range(-1,len(points) - 1):
                pygame.draw.line(self.game_display,color,points[point_number].int_pair(),points[point_number + 1].int_pair(),width)
        elif style=="points":
            for point in points:
                pygame.draw.circle(self.game_display,color,point.int_pair(),width)
class Joint(Line):
    def __init__(self,screen_size,game_display):
